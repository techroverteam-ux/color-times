...
export function BookingsClient({
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status: newStatus }: { id: string; status: BookingStatus }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <p className="text-sm text-muted-foreground">{pagination.total} bookings</p>
      </div>
          ...
          <tbody>
            {bookings.map((booking) => (
              <tr key={booking._id} className="border-b border-border last:border-0">
                ...
                <td className="px-4 py-3 text-muted-foreground">{booking.product?.name ?? "—"}</td>
                <td className="px-4 py-3 text-xs text-muted-foreground">
                  {new Date(booking.rentalStartDate).toLocaleDateString("en-IN")} &rarr;{" "}
                  {new Date(booking.rentalEndDate).toLocaleDateString("en-IN")}
                </td>
                <td className="px-4 py-3">&#8377;{booking.totalAmount.toLocaleString("en-IN")}</td>
                <td className="px-4 py-3">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      if (value && value !== booking.status) {
                        updateStatusMutation.mutate({ id: booking._id, status: value as BookingStatus });
                      }
                    }}
                  >
